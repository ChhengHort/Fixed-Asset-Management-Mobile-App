...
class _AssetDetailScreenState extends State<AssetDetailScreen> {
  ...
  // Always read the live asset from the provider so status stays in sync
  // with the list screen after popping back.
  FixedAsset _liveAsset(FixedAssetProvider provider) =>
      provider.findById(widget.asset.id) ?? widget.asset;

  Future<void> _handleApprove(FixedAssetProvider provider) async {
    setState(() => _isUpdating = true);
    await provider.approveAsset(widget.asset.id);
  ...
  Future<void> _handleReject(FixedAssetProvider provider) async {
    setState(() => _isUpdating = true);
    await provider.rejectAsset(widget.asset.id);
    setState(() => _isUpdating = false);
  ...
  @override
  Widget build(BuildContext context) {
    return Consumer<FixedAssetProvider>(
      builder: (context, provider, _) {
        final asset = _liveAsset(provider);
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          // ✅ Live status badge — updates instantly
